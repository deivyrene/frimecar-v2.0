...
//
		   ...
		   // @$pdf->Cell(40,7,utf8_decode(''),0,0,'C');
            ...
            {	
            	...
            	@$total_cobrar = ($fila_['monto_quincena'] + $fila_['bonificacion'] + $fila_['cesta_ticket'] + $fila_['feriados_trabajados'] + $fila_['h_extras_diur'] + $fila_['h_extras_noct'] + $fila_['otros_pagos'] ) - $total_ret;
            }
			
			$pdf->Cell(90,6,utf8_decode('He recibido de: '.$row_empresa['nombre_empresa'].', la cantidad de: Bs. '),0,0,'L',0);

			$pdf->SetFont('Arial','B',8);
			$pdf->Cell(46,6,utf8_decode(@$total_cobrar),0,0,'L',0);
			...
			$pdf->Cell(31,6,utf8_decode(@$mensual),0,0,'L',0);
			...
			$pdf->Cell(30,6,utf8_decode(@$mensual),0,0,'L',0);*/
											...
											select id_personal, 
											       ...
											       '0.00' nombre_personal 
																						   from tbl_personal where id_personal = $id_personal
																							                   and estatus_personal = 1 union 
											select id_personal, 
												   'OTROS PAGOS' apellido_personal, 
											       (select '0.00' otros_pagos from tbl_quincena where fk_personal_quincena = $id_personal and 
																			                    periodo_mes_quincena = '$descrip_periodo' and
											                                                    fk_periodo_quincena = $id_periodo) as quincena,
											       '0' cant_, 
											       '0.00' nombre_personal 
